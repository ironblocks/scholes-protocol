...
contract Deploy is Script {
    ...
    function run() external {
...

        {
            ...
            require(
                keccak256("WETH") ==
                    keccak256(
                        abi.encodePacked(
                            options.getUnderlyingToken(oid).symbol()
                        )
                    ),
                "WETH symbol mismatch"
            ); // Check
            require(
                opt.expiration == options.getExpiration(oid),
                "Expiration mismatch"
            ); // Double-check

            if (
                keccak256(bytes(vm.envString("CREATE_TEST_ORDERS"))) ==
                keccak256(bytes("yes"))
            ) {
        ...
        // Some more test options
        {
            ...
            console.log(
                "Long Option Id:",
                obList.getOrderBook(obList.getLength() - 1).longOptionId()
            );
        }
