...
The protocol needs an oracle (price feed), which would be used mainly for liquidations. The oracle should be able to provide pricing of the underlying assets. Even though the options' premiums are determined by the market forces in the marketplace, the protocol calculates its own metrics used for the purpose of determining entry and liquidation collateralization levels. 
